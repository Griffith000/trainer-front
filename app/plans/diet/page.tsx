...
export default function DietPlansPage() {
  const { data: plans, isLoading, error } = useQuery({
    queryKey: ["diet-plans"],
    queryFn: async () => {
      const { data } = await api.get("/api/chat/diet-plans/");
      return data;
    }
  });
  ...
  if (error) {
    return (
      <div className="flex flex-col h-[50vh] items-center justify-center text-destructive border border-destructive/20 rounded-xl bg-destructive/5 p-8 max-w-sm mx-auto mt-10">
        ...
        <p className="font-medium text-center">Failed to load diet plans.</p>
  ...
  if (!plans || plans.length === 0) {
    return (
      <div className="flex flex-col h-[50vh] items-center justify-center text-muted-foreground text-center bg-muted/30 rounded-xl border border-dashed p-10 max-w-lg mx-auto mt-10">
        <p className="text-lg font-medium text-foreground">No diet plans yet</p>
        <p className="text-sm mt-2">Ask the AI Coach to generate a diet plan for you, and it will appear here!</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {plans.map((plan: any) => (
        <PlanListItem key={plan.id} plan={plan} type="diet" />
      ))}
    </div>
  );
}
